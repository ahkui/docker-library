...
RUN cd openpose/build \
    ...
    cmake --quiet -D USE_OPENCV=ON -D USE_NCCL=ON -D BUILD_PYTHON=ON .. \
    ;else \
    echo ENABLE NVIDIA $JUPYTERHUB_ENABLE_NVIDIA && \
    cmake --quiet -D BUILD_PYTHON=ON -D GPU_MODE=CPU_ONLY .. || true && \
    ...
    && \
    cmake --quiet -D BUILD_PYTHON=ON -D GPU_MODE=CPU_ONLY .. \
    ;fi \
    && \
    make --quiet -j`nproc` \
    && \
